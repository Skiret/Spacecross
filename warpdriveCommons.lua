...
local function writePosition()
  ...
  dim = getDimension()
  if(cx ~= lx or cy ~= ly or cz ~= lz or ship_lastPosition.where ~= dim) then
    local req = require("internet").request(addr, {ship_name(), dim, cx, cy, cz})
  end
  end)
end
...
function ship_save()
  ...
  ship_lastPosition.where = getDimension()
